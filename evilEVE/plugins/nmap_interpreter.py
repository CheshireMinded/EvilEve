...
from plugins.nmap_plugin import run_nmap_scan, parse_nmap_xml
from plugins.next_tool_queue import queue_next_tools
...
def simulate_phase(attacker, phase, target_ip):
    ...
    if tool == "metasploit":
        ...
        xml_path = run_nmap_scan(target_ip)
        parsed = parse_nmap_xml(xml_path)
        suggestions = queue_next_tools(attacker, parsed.get("open_ports", []))
        result.update({
            "tool": tool, "args": [target_ip], "pid": None, "launched": False,
            "elapsed": 0.0, "stdout_snippet": "", "stderr_snippet": "",
            "deception_triggered": False, "monitored_status": "plugin", "exit_code": None,
            "bias": selected_bias, "tool_reason": bias_tool_reason,
            "log_warning": f"Nmap scan complete (xml: {xml_path})",
            "nmap_services": parsed.get("open_ports", []),
            "nmap_followups": suggestions
        })
